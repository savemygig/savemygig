...
export function initAccounts(core: ChecklistCore, me?: any): AccountLayer {
  ...
  function makePillDragHandle(pill: HTMLElement, name: string): HTMLButtonElement {
    ...
    b.setAttribute('aria-label', 'Arraste para reordenar ' + name);
  ...
  function makePillPencil(pill: HTMLElement, span: HTMLElement, id: string, name: string): HTMLButtonElement {
    ...
    b.setAttribute('aria-label', 'Renomear ' + name);
  ...
  function renderPills() {
    ...
    reg.lists.forEach((l) => {
      ...
      core.attachExplainer(pill, () => 'Toque para trocar para esta lista. Marcações, notas e edições ficam salvas em cada lista separadamente.');
      ...
      if (l.id !== reg.active) {
        ...
        x.className = 'ls-x';
        x.textContent = '×';
        x.setAttribute('aria-label', 'Apagar a lista ' + l.n);
  ...
  function createList(name: string) {
    ...
    if (reg.lists.length >= MAX_LISTS) {
      if (lsMsg) { lsMsg.textContent = 'Sete listas é o limite. Apague uma que você não usa mais.'; lsMsg.className = 'acct-msg err'; }
      return;
    }
        ...
        callback: async (resp: { credential: string }) => {
          ...
          try {
            ...
            if (d && d.ok && d.email) {
              ...
              if (fromGate) {
                ...
                if (core.pendingWant() === 'pdf') {
                  if (!core.printCurrentList() && backupMsg) {
                    backupMsg.textContent = 'Você está cadastrado. Seu navegador bloqueou a janela do download: toque em Baixar lista de novo e libere pop-ups.';
              ...
              if (msgEl) { msgEl.textContent = 'O acesso pelo Google não foi. Use o seu e-mail.'; msgEl.className = (fromGate ? 'unlock-msg' : 'acct-msg') + ' err'; }
            }
          } catch (e) {
            const msgEl = fromGate ? unlockMsg : acctMsg;
            if (msgEl) { msgEl.textContent = 'Sem conexão agora. Sua lista está segura neste aparelho.'; msgEl.className = (fromGate ? 'unlock-msg' : 'acct-msg') + ' err'; }
          }
        },
      });
  ...
  if (acctForm) {
    ...
    acctForm.addEventListener('submit', async (ev) => {
      ...
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(email)) {
        if (acctMsg) { acctMsg.textContent = 'Esse e-mail não parece certo. Confira e tente de novo.'; acctMsg.className = 'acct-msg err'; }
        inp.focus();
        return;
      }
      busy = true;
      if (acctMsg) { acctMsg.textContent = 'Enviando seu link...'; acctMsg.className = 'acct-msg'; }
      try {
        ...
        if (res.status === 429) {
          if (acctMsg) { acctMsg.textContent = 'Já tem um link a caminho. Veja a caixa de entrada, ou espere alguns minutos por um novo.'; acctMsg.className = 'acct-msg err'; }
        } else if (res.ok) {
          if (acctMsg) { acctMsg.textContent = 'Enviado para ' + email + '. Abra neste aparelho e toque no botão: isso conecta você aqui.'; acctMsg.className = 'acct-msg ok'; }
        } else {
          if (acctMsg) { acctMsg.textContent = 'Não deu para enviar agora, o problema é do nosso lado. Tente de novo em um minuto.'; acctMsg.className = 'acct-msg err'; }
        }
      } catch (e) {
        if (acctMsg) { acctMsg.textContent = 'Sem conexão. Tente de novo quando tiver sinal.'; acctMsg.className = 'acct-msg err'; }
      }
      busy = false;
    });
  }
  ...
  if (acctProfileForm) {
    ...
    acctProfileForm.addEventListener('submit', async (ev) => {
      ...
      if (!artist) {
        if (acctProfileMsg) { acctProfileMsg.textContent = 'Coloque seu nome artístico ou primeiro nome, para a gente saber como te chamar.'; acctProfileMsg.className = 'acct-msg err'; }
        aInp.focus();
        return;
      }
      profileBusy = true;
      try {
        ...
        if (res.ok && d && d.ok) {
          ...
          updateAcctUI();
          if (acctMsg2) {
            acctMsg2.textContent = instagram ? 'Salvo. Vamos seguir seu trabalho com prazer.' : 'Salvo.';
          ...
          acctProfileMsg.textContent = 'Não deu para salvar agora, o problema é do nosso lado. Tente de novo em um minuto.'; acctProfileMsg.className = 'acct-msg err';
        }
      } catch (e) {
        if (acctProfileMsg) { acctProfileMsg.textContent = 'Sem conexão. Tente de novo quando tiver sinal.'; acctProfileMsg.className = 'acct-msg err'; }
      }
      profileBusy = false;
    });
  }
  ...
  if (delAcctBtn) {
    ...
    const delFineWarn = 'Isso não tem volta. Sua conta e as listas salvas e sincronizadas serão apagadas.';
    const disarm = () => {
      armed = false;
    ...
    delAcctBtn.addEventListener('click', async () => {
      ...
      disarm();
      if (acctMsg2) { acctMsg2.textContent = 'Apagando...'; acctMsg2.className = 'acct-msg'; }
      try {
        ...
        if (res.ok && d && d.ok) {
          ...
          if (reg.active !== 'main' && reg.lists.some((l) => l.id === 'main')) { switchList('main'); return; }
          updateAcctUI();
          if (acctMsg) { acctMsg.textContent = 'Conta apagada. Tudo do nosso lado sumiu; a lista deste aparelho continua sua.'; acctMsg.className = 'acct-msg ok'; }
        } else if (acctMsg2) {
          acctMsg2.textContent = 'Não deu para apagar agora, o problema é do nosso lado. Tente de novo em um minuto.'; acctMsg2.className = 'acct-msg err';
        }
      } catch (e) {
        if (acctMsg2) { acctMsg2.textContent = 'Sem conexão. Tente de novo quando tiver sinal.'; acctMsg2.className = 'acct-msg err'; }
      }
    });
  }
  ...
  function setListDeleteMode(on: boolean) {
    ...
    if (lsDelLabel) lsDelLabel.textContent = on ? 'Pronto' : 'Apagar';
  ...
  function setListMoveMode(on: boolean) {
    ...
    if (lsMoveLabel) lsMoveLabel.textContent = on ? 'Pronto' : 'Gerenciar';
  }
  ...
  // used; open the card so asking again is one tap away.
  {
    ...
    if (sp.get('signedin') === '1') {
      if (acctMsg) { acctMsg.textContent = 'Conectado. Suas listas te acompanham agora.'; acctMsg.className = 'acct-msg ok'; }
      sp.delete('signedin');
      ...
      if (acctMsg) { acctMsg.textContent = 'Esse link de acesso expirou ou já foi usado. Peça um novo aqui embaixo.'; acctMsg.className = 'acct-msg err'; }
      sp.delete('signin');
      history.replaceState(null, '', location.pathname + (sp.toString() ? '?' + sp.toString() : '') + location.hash);
    }
  }
